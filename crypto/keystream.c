                      ...
                      int err )
{
    ...
    if ( !err )
    {
        ...
        if ( 0 == atomic_dec_return( &ks->in_flight ) )
            ks->state = KS_ACTIVE;
            ...
            struct file *filp )
{
    ...
    if ( 0 == --ks->use_count ) {
        if ( wait_event_interruptible( ks->waitq,
                                       !atomic_read( &ks->in_flight ) ) )
            return -ERESTARTSYS;
          ...
          loff_t *offp )
{
    ...
    ret = -EBUSY;
    if ( ks->state == KS_UNCONFIGURED || !ks->buf )
        goto out;
    ...
    while ( written < len && ( len - written ) >= ivsize )
    {
        ...
        sg_set_buf( &req->src, ks->buf + ks->wptr, clen );
        memset( ks->buf + ks->wptr, 0, clen );
        ...
        ablkcipher_request_set_crypt( req->areq, &req->src, &req->src,
                                      ks->ks_sz, req->iv );
...
ks_generator_release( struct config_item *item )
{
    ...
    while ( atomic_read( &ks->in_flight ) )
        cpu_relax();
                        ...
                        const char *name )
{
    ...
    snprintf( ks->cache_name, sizeof( ks->cache_name ) - 1, "%s-cache", name );

    spin_lock_init( &ks->lock );

    atomic_set( &ks->in_flight, 0 );
    ks->rptr        = ks->wptr = 0;
    ks->buf         = NULL;
    ks->key         = NULL;
    ks->alg[ 0 ]    = '\0';
    ...
    ret = -ENOMEM;
    ks->req_cache   = kmem_cache_create( ks->cache_name,
